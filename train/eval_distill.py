...
def main():
    ...
    # Chemin vers le checkpoint du modèle distillé 5500
    checkpoint_path = r"i:\qwen3_asr\Qwen3-ASR\qwen3-asr-distilled\checkpoint-5500"
    
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"Checkpoint not found at {checkpoint_path}")
        
    # Le checkpoint sauvegardé par Trainer manque de certains fichiers (comme preprocessor_config.json)
    # On charge donc le processor depuis le modèle original, et le modèle depuis le checkpoint
    from transformers import AutoModel, AutoProcessor
    
    teacher_model_id = "Qwen/Qwen3-ASR-0.6B"
    processor = AutoProcessor.from_pretrained(teacher_model_id, fix_mistral_regex=True)
